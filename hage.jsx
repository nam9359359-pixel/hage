      ...
      <div style={{
        ...
        <div style={{ marginBottom: 16 }}>
          <div style={{
            ...
            <input
              type="password" value={pw} placeholder="비밀번호 입력"
        ...
        <div style={{ marginTop: 24, padding: "14px", background: "#0f172a", borderRadius: 10, fontSize: 12, color: "#475569", lineHeight: 1.8 }}>
          <div>👥 <strong style={{ color: "#94a3b8" }}>팀원</strong>: 수정 사항 열람만 가능</div>
...
function Modal({ item, onClose, onSave }) {
  const [form, setForm] = useState(item || {
    category: "로그인/회원가입", page: "", desc: "", detail: "", status: "대기"
  });
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        borderRadius: 18, padding: "30px 28px", width: "100%", maxWidth: 500,
        boxShadow: "0 25px 60px rgba(0,0,0,0.5)", maxHeight: "90vh", overflowY: "auto"
      }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
          <h2 style={{ fontSize: 18, fontWeight: 700 }}>{item ? "✏️ 항목 수정" : "➕ 새 항목 추가"}</h2>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div>
            ...
            <textarea value={form.desc} onChange={e => set("desc", e.target.value)} placeholder="수정해야 할 내용을 입력하세요" rows={3}
          ...
          <div>
            ...
            <input value={form.detail} onChange={e => set("detail", e.target.value)} placeholder="예: 최대 20자 제한, 다음·카카오·구글 추가" style={inputStyle} />
          </div>
          <div>
              ...
              {STATUS_LIST.map(s => {
                ...
                return (
                  <button key={s} onClick={() => set("status", s)} style={{
                    ...
                    color: active ? st.color : "#64748b", fontWeight: 600, fontSize: 13, cursor: "pointer",
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 24 }}>
          <button onClick={onClose} style={{
            flex: 1, padding: "12px", border: "1px solid #334155", borderRadius: 10,
...
function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const result = await window.storage.get("bugtracker-items", true);
        if (result && result.value) {
          setItems(JSON.parse(result.value));
        } else {
          setItems(INITIAL_ITEMS);
          await window.storage.set("bugtracker-items", JSON.stringify(INITIAL_ITEMS), true);
        }
      } catch {
        setItems(INITIAL_ITEMS);
      }
  ...
  const save = async (newItems) => {
    setItems(newItems);
    try { await window.storage.set("bugtracker-items", JSON.stringify(newItems), true); } catch {}
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0f172a", fontFamily: "'Noto Sans KR',sans-serif" }}>
      <style>{css}
        {`.item-row:hover{background:#1e293b !important;}
        ...
        <div style={{ maxWidth: 1000, margin: "0 auto", padding: "0 20px", display: "flex", alignItems: "center", gap: 16, height: 60 }}>
          ...
          <div style={{
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="검색..." style={{
              ...
              color: "#f1f5f9", fontSize: 13, width: 140, fontFamily: "'Noto Sans KR',sans-serif"
            }} />
          ...
          <button onClick={() => { setRole(null); setItems([]); setLoading(true); }} style={{
      ...
      <div style={{ maxWidth: 1000, margin: "0 auto", padding: "24px 20px" }}>
        ...
        <div style={{ display: "flex", gap: 12, marginBottom: 24, flexWrap: "wrap" }}>
          {[
            { label: "전체", val: total, color: "#94a3b8", bg: "#1e293b" },
            ...
            <div key={s.label} style={{
              background: s.bg, border: `1px solid ${s.color}33`, borderRadius: 12,
          ...
          {role === "admin" && (
            <div style={{
              ...
              gap: 6, fontSize: 12, color: "#f59e0b"
            }}>
              🔑 관리자 모드
            </div>
          )}
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap", alignItems: "center" }}>
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            {["전체", ...STATUS_LIST].map(s => {
              ...
              const st = s !== "전체" ? statusStyle(s) : { bg: "#3b82f6", color: "#3b82f6", dot: "#3b82f6" };
              return (
                <button key={s} className="cat-btn" onClick={() => setFilterStatus(s)} style={{
                  ...
                  fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "'Noto Sans KR',sans-serif",
        ...
        {/* LIST */}
        {loading ? (
          <div style={{ textAlign: "center", color: "#475569", padding: 60 }}>불러오는 중...</div>
            ...
            <div key={cat} style={{ marginBottom: 24, animation: "fadein 0.3s ease" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10, paddingLeft: 4 }}>
                ...
                <span style={{
                  ...
                  border: "1px solid #334155", borderRadius: 10, padding: "1px 8px", fontWeight: 600
                }}>{catItems.length}건</span>
              </div>
                ...
                {catItems.map((item, idx) => {
                  ...
                  return (
                    <div key={item.id} className="item-row" style={{
                      ...
                      padding: "14px 18px",
                      borderBottom: idx < catItems.length - 1 ? "1px solid #334155" : "none",
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{
                          ...
                          background: "#1e3a5f", display: "inline-block", borderRadius: 5,
                          padding: "1px 7px", marginBottom: 5, maxWidth: "100%",
                        ...
                        {item.detail && (
                          <div style={{
                            ...
                            background: "#0f172a", display: "inline-block",
                            borderRadius: 5, padding: "2px 8px"
                          }}>💡 {item.detail}</div>
                        )}
                      </div>

                      {/* Status - 관리자/팀원 모두 변경 가능 */}
                      <select value={item.status} onChange={e => handleStatusChange(item.id, e.target.value)} style={{
                        background: st.bg + "33", border: `1px solid ${st.dot}55`,
                        borderRadius: 20, color: st.color, fontWeight: 700, fontSize: 11,
                      ...
                      {role === "admin" && (
                        <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                          <button className="action-btn" onClick={() => setModal(item)} style={{
                            padding: "5px 10px", background: "#334155", border: "none",
                            borderRadius: 7, color: "#94a3b8", fontSize: 12, cursor: "pointer",
                            fontFamily: "'Noto Sans KR',sans-serif", transition: "opacity 0.15s"
                          }}>✏️</button>
                          <button className="action-btn" onClick={() => setConfirmDel(item)} style={{
                            padding: "5px 10px", background: "#450a0a", border: "none",
      ...
      {confirmDel && (
        <div style={{
          ...
          <div style={{
            ...
            <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 8, textAlign: "center" }}>항목을 삭제할까요?</h3>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => setConfirmDel(null)} style={{
                flex: 1, padding: "11px", border: "1px solid #334155", borderRadius: 10,
