...
function LoginPage({ onLogin }) {
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(135deg, #0f172a 0%, #1e293b 50%, #0f172a 100%)", padding: "20px", fontFamily: "'Noto Sans KR', sans-serif" }}>
      <style>{`
      ...
      `}</style>
      <div style={{ width: "100%", maxWidth: 400, background: "#1e293b", border: "1px solid #334155", borderRadius: 20, padding: "40px 36px", boxShadow: "0 25px 60px rgba(0,0,0,0.5)", animation: shake ? "shake 0.4s ease" : "fadein 0.4s ease" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 20, background: "#0f172a", borderRadius: 12, padding: 4 }}>
          {[["viewer", "👥 팀원 보기"], ["admin", "🔑 관리자"]].map(([r, label]) => (
            <button key={r} onClick={() => { setRole(r); setErr(""); setPw(""); }} style={{ flex: 1, padding: "10px 0", borderRadius: 9, border: "none", cursor: "pointer", fontWeight: 600, fontSize: 13, background: role === r ? "#3b82f6" : "transparent", color: role === r ? "#fff" : "#64748b", transition: "all 0.2s" }}>{label}</button>
          ))}
        </div>
        <div style={{ marginBottom: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, background: "#0f172a", border: `1px solid ${err ? "#ef4444" : "#334155"}`, borderRadius: 12, padding: "12px 16px" }}>
        ...
        <button onClick={handleLogin} style={{ width: "100%", padding: "14px", border: "none", borderRadius: 12, background: "linear-gradient(135deg, #3b82f6, #2563eb)", color: "#fff", fontWeight: 700, fontSize: 15, cursor: "pointer", boxShadow: "0 4px 15px rgba(59,130,246,0.4)" }}>입장하기</button>
...
function Modal({ item, onClose, onSave }) {
  ...
  const inputStyle = { width: "100%", padding: "10px 14px", background: "#0f172a", border: "1px solid #334155", borderRadius: 10, color: "#f1f5f9", fontSize: 14, outline: "none" };
  return (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {STATUS_LIST.map(s => {
                const st = statusStyle(s); const active = form.status === s;
                return <button key={s} onClick={() => set("status", s)} style={{ flex: 1, padding: "8px 0", border: `2px solid ${active ? st.dot : "#334155"}`, borderRadius: 8, background: active ? st.bg + "33" : "transparent", color: active ? st.color : "#64748b", fontWeight: 600, fontSize: 13, cursor: "pointer", transition: "all 0.15s" }}><span style={{ marginRight: 4 }}>●</span>{s}</button>;
              })}
            </div>
          </div>
        </div>
...
export default function App() {
  ...
  useEffect(() => {
    if (!role) return;
    const colRef = collection(db, "bugtracker");
    const unsub = onSnapshot(colRef, async (snap) => {
      if (snap.empty) {
        // 최초 실행 시 초기 데이터 Firebase에 저장
        for (const item of INITIAL_ITEMS) {
          await setDoc(doc(db, "bugtracker", item.id), item);
        }
      } else {
  ...
  const handleSave = async (form) => {
    ...
    if (modal === "add") {
      const newId = String(Math.max(0, ...items.map(i => parseInt(i.id))) + 1);
      await setDoc(doc(db, "bugtracker", newId), { ...form, id: newId });
      showToast("✅ 항목이 추가되었습니다");
    } else {
      await updateDoc(doc(db, "bugtracker", form.id), form);
  ...
  const handleDelete = async (id) => {
    await deleteDoc(doc(db, "bugtracker", id));
    setConfirmDel(null);
    showToast("🗑️ 항목이 삭제되었습니다");
  };

  const handleStatusChange = async (id, newStatus) => {
    await updateDoc(doc(db, "bugtracker", id), { status: newStatus });
    ...
    <div style={{ minHeight: "100vh", background: "#0f172a", fontFamily: "'Noto Sans KR', sans-serif", color: "#f1f5f9" }}>
      ...
      <div style={{ background: "#1e293b", borderBottom: "1px solid #334155", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: 1000, margin: "0 auto", padding: "0 20px", display: "flex", alignItems: "center", gap: 16, height: 60 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 11, color: "#64748b", marginTop: 2 }}>실시간 동기화 🔥</p>
          </div>
          <div style={{ flex: 1 }} />
          ...
          {role === "admin" && (
            <button onClick={() => setModal("add")} style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 14px", background: "linear-gradient(135deg, #3b82f6, #2563eb)", border: "none", borderRadius: 8, color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer", whiteSpace: "nowrap" }}>+ 추가</button>
          )}
          <button onClick={() => { setRole(null); setItems([]); setLoading(true); }} style={{ padding: "7px 12px", background: "transparent", border: "1px solid #334155", borderRadius: 8, color: "#64748b", fontSize: 12, cursor: "pointer" }}>로그아웃</button>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1000, margin: "0 auto", padding: "24px 20px" }}>
        ...
        <div style={{ display: "flex", gap: 12, marginBottom: 24, flexWrap: "wrap" }}>
          ...
          {role === "admin" && (
            <div style={{ marginLeft: "auto", background: "#1e293b", border: "1px solid #334155", borderRadius: 12, padding: "8px 16px", display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: "#f59e0b" }}>🔑 관리자 모드</div>
          )}
        </div>
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 12, flexWrap: "wrap" }}>
          {["전체", ...STATUS_LIST].map(s => {
            ...
            return <button key={s} className="cat-btn" onClick={() => setFilterStatus(s)} style={{ padding: "5px 12px", borderRadius: 20, border: `1px solid ${active ? st.dot : "#334155"}`, background: active ? st.dot + "22" : "transparent", color: active ? st.color : "#64748b", fontSize: 12, fontWeight: 600, cursor: "pointer", transition: "all 0.15s" }}>{s === "전체" ? "전체 상태" : `● ${s}`}</button>;
          })}
        </div>
        <div style={{ display: "flex", gap: 6, marginBottom: 24, flexWrap: "wrap" }}>
          {CATEGORIES.map(c => (
            <button key={c} className="cat-btn" onClick={() => setFilterCat(c)} style={{ padding: "5px 12px", borderRadius: 20, border: `1px solid ${filterCat === c ? "#3b82f6" : "#334155"}`, background: filterCat === c ? "#3b82f633" : "transparent", color: filterCat === c ? "#60a5fa" : "#64748b", fontSize: 12, fontWeight: 600, cursor: "pointer", transition: "all 0.15s" }}>
              {c !== "전체" && CATEGORY_ICONS[c] && <span style={{ marginRight: 4 }}>{CATEGORY_ICONS[c]}</span>}{c}
            </button>
          ))}
        </div>
        ...
        {loading ? (
          <div style={{ textAlign: "center", color: "#475569", padding: 80 }}>
            ...
            <p>Firebase에서 불러오는 중...</p>
          </div>
        ) : Object.keys(groupedByCategory).length === 0 ? (
          <div style={{ textAlign: "center", color: "#475569", padding: 60 }}>
            <div style={{ fontSize: 40, marginBottom: 12 }}>🔍</div><p>해당하는 항목이 없습니다</p>
          </div>
        ) : (
            ...
            <div key={cat} style={{ marginBottom: 24, animation: "fadein 0.3s ease" }}>
              ...
              <div style={{ background: "#1e293b", border: "1px solid #334155", borderRadius: 14, overflow: "hidden" }}>
                {catItems.map((item, idx) => {
                  ...
                  return (
                    <div key={item.id} className="item-row" style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 18px", borderBottom: idx < catItems.length - 1 ? "1px solid #334155" : "none", transition: "background 0.15s" }}>
                      ...
                      <select value={item.status} onChange={e => handleStatusChange(item.id, e.target.value)} style={{ background: st.bg + "33", border: `1px solid ${st.dot}55`, borderRadius: 20, color: st.color, fontWeight: 700, fontSize: 11, padding: "4px 8px", cursor: "pointer", outline: "none", whiteSpace: "nowrap" }}>
      ...
      {confirmDel && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999, padding: 20 }}>
          <div style={{ background: "#1e293b", border: "1px solid #334155", borderRadius: 16, padding: "28px", maxWidth: 380, width: "100%", boxShadow: "0 25px 60px rgba(0,0,0,0.5)" }}>
            ...
            <p style={{ fontSize: 13, color: "#94a3b8", textAlign: "center", lineHeight: 1.6, marginBottom: 24 }}>"{confirmDel.desc.slice(0, 30)}{confirmDel.desc.length > 30 ? "..." : ""}"<br />삭제 후 복구할 수 없습니다.</p>
