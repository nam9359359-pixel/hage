...
import { doc, onSnapshot, collection, addDoc, deleteDoc, serverTimestamp, updateDoc } from 'firebase/firestore'
import { ACCOUNTS, STATUS_STYLE, STATUS_LIST } from '../constants.js'
import { logActivity, formatDate, dueDateStatus } from '../utils.js'
...
export default function ItemDetailPage({ user, navigate, onLogout, currentPage, itemId }) {
  ...
  const sendComment = async () => {
    ...
    // 담당자/작성자에게 알림 (본인 제외)
    const targets = new Set([item?.assignee, item?.createdBy].filter(id=>id&&id!==user.id))
    for (const toUserId of targets) {
      await addNotification({ db, toUserId, type:'comment', title:'새 댓글', body:`"${item?.desc?.slice(0,30)}" — ${user.name}: ${newComment.trim().slice(0,30)}`, itemId })
    }
    setNewComment('')
      ...
      <div style={{ maxWidth:760, margin:'0 auto', padding:'28px 20px' }}>
        ...
        <SectionCard style={{ marginBottom:24, padding:'24px 22px' }}>
          <div style={{ display:'flex', alignItems:'flex-start', gap:12, flexWrap:'wrap', marginBottom:16 }}>
            ...
            {item.dueDate && (() => { const ds=dueDateStatus(item.dueDate); return ds ? <span style={{ fontSize:12, padding:'2px 10px', background:ds.bg, border:`1px solid ${ds.border}`, borderRadius:50, color:ds.color, fontWeight:700 }}>📅 {ds.label}</span> : null })()}
              ...
              <div key={c.id} style={{ padding:'14px 18px', borderBottom:idx<comments.length-1?'1px solid #192640':'none' }}>
                ...
                {isMe && (
                  <div style={{ paddingLeft:36, marginTop:4 }}>
                    <button onClick={()=>deleteComment(c.id)} style={{ padding:'2px 8px', background:'rgba(239,68,68,0.08)', border:'none', borderRadius:5, color:'#f87171', fontSize:11, cursor:'pointer', fontFamily:"'Noto Sans KR',sans-serif" }}>삭제</button>
                  </div>
                )}
              </div>
            )
