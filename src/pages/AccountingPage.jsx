...
const CAT_COLORS = {'매출':'#1D9E75','용역수입':'#5DCAA5','이자수입':'#9FE1CB','기타수입':'#085041','식비/접대':'#D85A30','교통':'#378ADD','인건비':'#7F77DD','마케팅':'#D4537E','장비/소모품':'#BA7517','임대료':'#E24B4A','통신비':'#639922','세금/공과금':'#888780','기타':'#B4B2A9','구독 서비스':'#534AB7'}
const COLOR_POOL = ['#1D9E75','#378ADD','#7F77DD','#D4537E','#BA7517','#E24B4A','#639922','#D85A30','#534AB7','#5DCAA5']

// 과세 구분별 부가세 계산
// taxable: 공급가액 기준 (입력액 × 0.1 = 부가세)
// exempt: 면세 (부가세 없음)
// inclusive: 부가세포함 (입력액 ÷ 1.1 × 0.1 = 부가세)
const calcVAT = (amount, tax) => {
  if (tax === 'taxable') return Math.round(amount * 0.1)
  if (tax === 'inclusive') return Math.round(amount - amount / 1.1)
  return 0
}
const supplyAmt = (amount, tax) => {
  if (tax === 'inclusive') return Math.round(amount / 1.1)
  return amount
}
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:4 }}>
        <h3 style={{ fontSize:16, fontWeight:700, color:'#f1f5f9' }}>{initial ? '✏️ 내역 수정' : '➕ 내역 추가'}</h3>
...
export default function AccountingPage({ user, navigate, onLogout, currentPage }) {
  ...
  const addCat = async (type) => {
    ...
    setNewCat(n=>({...n,[type]:''})); showToast('✅ 카테고리가 추가되었습니다')
  }
  ...
  const exportExcel = () => {
    ...
    const ledgerData = [
      ['날짜','구분','카테고리','거래처','금액','공급가액','부가세','업무구분','메모'],
      ...monthEntries.map(e=>[e.date, e.type==='income'?'수입':'지출', e.cat, e.client||'', e.amount, supplyAmt(e.amount,e.tax), calcVAT(e.amount,e.tax), e.biz==='biz'?'업무용':'개인용', e.memo||'']),
      ...fixedCosts.map(f=>['-','지출(고정)',f.cat,'',f.amount,f.amount,0,'업무용',f.name])
    ]
    const ws1 = XLSX.utils.aoa_to_sheet(ledgerData)
    ws1['!cols'] = [12,8,12,12,14,14,12,8,16].map(w=>({wch:w}))
    XLSX.utils.book_append_sheet(wb, ws1, '거래내역')
    const ws2 = XLSX.utils.aoa_to_sheet([['항목','금액'],['총 매출',revenue],['총 지출',totalExp],['순이익',profit],['납부 부가세',Math.round(vatPay)],['예산',budget||'-'],['예산 소진율',budget?budgetPct+'%':'-']])
    ws2['!cols'] = [{wch:18},{wch:16}]
    XLSX.utils.book_append_sheet(wb, ws2, '요약')
    const ws3 = XLSX.utils.aoa_to_sheet([['카테고리','지출액','비율'],...catChartData.sort((a,b)=>b.amount-a.amount).map(({cat,amount})=>[cat,amount,totalExp?Math.round(amount/totalExp*100)+'%':'0%'])])
  ...
  const TABS = [{id:'dashboard',label:'대시보드'},{id:'calendar',label:'달력 뷰'},{id:'ledger',label:'수입/지출'},{id:'vat',label:'부가세'},{id:'fixed',label:'고정비'},{id:'categories',label:'카테고리'}]
          ...
          {calSelected && (
            <div style={cardStyle}>
              <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:14 }}>
                <span style={{ fontSize:14, fontWeight:600, color:'#cbd5e1' }}>{parseInt(calSelected.split('-')[2])}일 내역</span>
                <span style={{ fontSize:12, color:'#64748b' }}>({calSelEntries.length}건)</span>
              ...
              {calSelEntries.length>0&&(
                <div style={{ display:'flex', justifyContent:'space-between', paddingTop:10, borderTop:'1px solid #1e2e50', fontSize:12, marginTop:4 }}>
                  <span style={{ color:'#64748b' }}>
                    {calSelEntries.filter(e=>e.type==='income').reduce((s,e)=>s+e.amount,0)>0&&`수입 ${fmt(calSelEntries.filter(e=>e.type==='income').reduce((s,e)=>s+e.amount,0))}`}
                    {calSelEntries.filter(e=>e.type==='income').length>0&&calSelEntries.filter(e=>e.type==='expense').length>0&&' / '}
                    {calSelEntries.filter(e=>e.type==='expense').reduce((s,e)=>s+e.amount,0)>0&&`지출 ${fmt(calSelEntries.filter(e=>e.type==='expense').reduce((s,e)=>s+e.amount,0))}`}
                  </span>
                </div>
              )}
            </div>
        ...
        {tab==='vat' && <>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
            <div style={cardStyle}>
              ...
              <div style={{ fontSize:11, color:'#475569', marginTop:12, lineHeight:1.7 }}>과세·부가세포함 항목에 적용됩니다<br/>부가세포함: 총액 ÷ 1.1 × 0.1 = 부가세</div>
            </div>
            <div style={cardStyle}>
