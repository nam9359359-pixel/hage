...
function calcYearlyAmt(sub) {
  return calcMonthlyAmt(sub)*12
}
...
function PeriodLabel({ sub }) {
  ...
  const months = calcSubMonths(sub)
  const label = months>=12 ? `${Math.round(months/12)}년 구독` : `${months}개월 구독`
  return <span style={{ fontSize:10, padding:'1px 7px', borderRadius:99, background:'rgba(167,139,250,0.12)', color:'#a78bfa', border:'1px solid rgba(167,139,250,0.2)' }}>{label}</span>
}
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:12, maxHeight:'80vh', overflowY:'auto' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
        ...
        <div>
          <label style={lbl}>
            {form.period==='monthly'?'월 금액':form.period==='yearly'?'연 금액':'총 금액'} (원) *
          </label>
          <input type="number" value={form.amount} onChange={e=>set('amount',e.target.value)} placeholder="0" style={iS}/>
        </div>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
        ...
        {form.period!=='custom' ? (
          ...
          <div><label style={lbl}>메모 (선택)</label><input value={form.memo} onChange={e=>set('memo',e.target.value)} placeholder="3년 약정 등" style={iS}/></div>
        )}
      </div>
      ...
      {form.period==='custom' && (
        <div style={{ background:'rgba(124,58,237,0.06)', border:'1px solid rgba(124,58,237,0.2)', borderRadius:9, padding:12 }}>
          <div style={{ fontSize:12, color:'#a78bfa', fontWeight:600, marginBottom:10 }}>📅 구독 기간 설정</div>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
          ...
          {form.startDate && form.endDate && (
            <div style={{ marginTop:10, fontSize:12, color:'#64748b', lineHeight:1.8 }}>
              총 기간: <span style={{ color:'#a78bfa', fontWeight:600 }}>{months}개월 ({months>=12?`${Math.floor(months/12)}년 ${months%12?months%12+'개월':''}`:months+'개월'})</span>
            </div>
          )}
        </div>
      )}
      ...
      {/* 비용 미리보기 */}
      {amt>0 && (
        <div style={{ background:'#0a1120', borderRadius:8, padding:'12px', fontSize:12, color:'#64748b', lineHeight:2 }}>
          <div>월 환산 비용: <span style={{ color:'#a78bfa', fontWeight:600 }}>{fmt(monthly)}/월</span></div>
          <div>연간 비용: <span style={{ color:'#a78bfa', fontWeight:600 }}>{fmt(yearly)}/년</span></div>
          {form.period==='custom' && months>0 && (
            <div>총 구독 비용: <span style={{ color:'#c084fc', fontWeight:600 }}>{fmt(amt)} ({months}개월)</span></div>
          )}
        </div>
      )}
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
      ...
      <div style={{ padding:'12px 14px', background:'rgba(83,74,183,0.08)', border:'1px solid rgba(83,74,183,0.2)', borderRadius:9 }}>
        <label style={{ display:'flex', alignItems:'center', gap:8, cursor:'pointer', marginBottom:form.isSubscription?10:0 }}>
          ...
          <span style={{ fontSize:11, color:'#64748b' }}>체크 시 구독 현황에 표시</span>
        </label>
        {form.isSubscription && (
          <div style={{ display:'flex', gap:8 }}>
            {[['monthly','월 구독'],['yearly','연 구독'],['custom','사용자 지정']].map(([v,l])=>(
...
export default function AccountingPage({ user, navigate, onLogout, currentPage }) {
  ...
  useEffect(() => {
    ...
    const u4 = onSnapshot(doc(db,'accounting_settings','main'), snap => {
      if (snap.exists()) { const d=snap.data(); if(d.incCats)setIncCats(d.incCats); if(d.expCats)setExpCats(d.expCats); if(d.budget!==undefined)setBudgetState(d.budget) }
    })
    return () => { u1(); u2(); u3(); u4() }
  }, [])
  ...
  const annualTotal = monthlyTotal*12 + yearlyTotal + customSubs.reduce((s,sub)=>s+calcYearlyAmt(sub),0)

  // 종료/갱신 임박 (90일 이내)
  const alertSubs = allSubs.filter(s=>s.active!==false).map(s=>({...s,days:daysUntilEnd(s)})).filter(s=>s.days!==null&&s.days<=90).sort((a,b)=>a.days-b.days)
  ...
  const fixedTotal = fixedCosts.filter(f=>!f.isSubscription).reduce((s,f)=>s+f.amount,0)
  const fixedSubTotal = fixedCosts.filter(f=>f.isSubscription).reduce((s,f)=>s+f.amount,0)
  const totalExp = varExp + fixedTotal + fixedSubTotal
  ...
  const expData = months6.map(({y,m})=>entries.filter(e=>{const d=new Date(e.date);return d.getFullYear()===y&&d.getMonth()===m&&e.type==='expense'}).reduce((s,e)=>s+e.amount,0)+fixedTotal+fixedSubTotal)
  ...
  const toggleSubActive = async s => {
    if (s.source==='fixed') { await updateDoc(doc(db,'accounting_fixed',s.id.replace('fx_','')),{active:!s.active}); }
  ...
  const exportExcel = () => {
    ...
    const ws2 = XLSX.utils.aoa_to_sheet([
      ...
      ['구독 현황',''],['월 구독 합계',monthlyTotal],['연 구독 합계',yearlyTotal],['사용자 지정 구독(월환산)',Math.round(customMonthly)],['연간 구독 총액',annualTotal],['월 예상 구독 고정 지출',totalMonthlyEquivalent],
    ])
    ws2['!cols']=[{wch:22},{wch:16}]
    XLSX.utils.book_append_sheet(wb,ws2,'요약')
    const ws3 = XLSX.utils.aoa_to_sheet([
      ['서비스명','구독주기','금액','월 환산','카테고리','시작일','종료/갱신일','출처','상태'],
      ...allSubs.map(s=>[s.name,s.period==='monthly'?'월':s.period==='yearly'?'연':'사용자지정',s.amount,calcMonthlyAmt(s),s.subCat||'',s.startDate||'',s.endDate||s.renewalDate||'',s.source==='fixed'?'고정비':'구독',s.active!==false&&!isExpired(s)?'활성':'비활성'])
    ])
    ws3['!cols']=[{wch:20},{wch:12},{wch:14},{wch:14},{wch:12},{wch:12},{wch:14},{wch:8},{wch:8}]
      ...
      <div style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 0', borderBottom:idx<total-1?'1px solid #192640':'none', opacity:inactive?0.55:1 }}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:3, flexWrap:'wrap' }}>
            ...
            {s.source==='fixed'&&<span style={{ fontSize:10, padding:'1px 6px', borderRadius:99, background:'rgba(255,255,255,0.06)', color:'#64748b' }}>📌 고정비</span>}
            {expired&&<span style={{ fontSize:10, padding:'1px 6px', borderRadius:99, background:'rgba(239,68,68,0.1)', color:'#f87171' }}>종료됨</span>}
            <StatusBadge sub={s}/>
          </div>
          {s.memo&&<div style={{ fontSize:11, color:'#475569' }}>{s.memo}</div>}
          {s.period==='custom'&&s.startDate&&s.endDate&&(
            <div style={{ fontSize:11, color:'#475569' }}>{s.startDate} ~ {s.endDate} ({calcSubMonths(s)}개월)</div>
          )}
        </div>
        ...
        <div style={{ display:'flex', gap:5, flexShrink:0 }}>
          <button onClick={()=>toggleSubActive(s)} style={{ padding:'4px 8px', background:inactive?'rgba(34,197,94,0.1)':'rgba(255,255,255,0.05)', border:`1px solid ${inactive?'rgba(34,197,94,0.2)':'#1e2e50'}`, borderRadius:6, color:inactive?'#4ade80':'#64748b', fontSize:11, cursor:'pointer', fontFamily:"'Noto Sans KR',sans-serif" }}>{inactive?'활성화':'해지'}</button>
          {s.source!=='fixed'&&<button onClick={()=>setSubModal(s)} style={{ padding:'4px 9px', background:'rgba(124,58,237,0.1)', border:'1px solid rgba(124,58,237,0.2)', borderRadius:6, color:'#a78bfa', fontSize:11, cursor:'pointer' }}>수정</button>}
          {s.source!=='fixed'&&<button onClick={()=>delSub(s.id)} style={{ padding:'4px 8px', background:'rgba(239,68,68,0.08)', border:'none', borderRadius:6, color:'#f87171', fontSize:11, cursor:'pointer' }}>삭제</button>}
          {s.source==='fixed'&&<span style={{ fontSize:11, color:'#475569', padding:'4px 8px' }}>고정비에서 수정</span>}
        </div>
      </div>
    )
  }
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#0d1526', fontFamily:"'Noto Sans KR',sans-serif", color:'#e2e8f0' }}>
      ...
      <div style={{ maxWidth:1100, margin:'0 auto', padding:'24px 20px' }}>
        ...
        {tab==='dashboard'&&<>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,minmax(0,1fr))', gap:10, marginBottom:18 }}>
            ...
            <StatCard label="연간 구독 총액" value={fmt(annualTotal)} color="#a78bfa" sub={`${activeSubs.length}개 서비스`}/>
            <StatCard label="월 예상 구독 고정 지출" value={fmt(totalMonthlyEquivalent)} color="#c084fc" sub="월+연÷12+사용자지정 월환산"/>
          </div>
          {expiredSubs.length>0&&(
            <div style={{ background:'rgba(239,68,68,0.06)', border:'1px solid rgba(239,68,68,0.2)', borderRadius:12, padding:'12px 16px', marginBottom:14 }}>
              <div style={{ fontSize:12, fontWeight:700, color:'#f87171', marginBottom:8 }}>🔴 종료된 구독 ({expiredSubs.length}건) — 비활성 처리를 권장합니다</div>
                  ...
                  <div key={e.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'11px 0', borderBottom:idx<filtered.length-1?'1px solid #192640':'none' }}>
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ display:'flex', alignItems:'center', gap:5, marginBottom:2, flexWrap:'wrap' }}>
                        ...
                        {e.isSubscription&&<span style={{ fontSize:10, padding:'1px 6px', borderRadius:99, background:'rgba(124,58,237,0.15)', color:'#a78bfa' }}>{e.subPeriod==='yearly'?'연구독':e.subPeriod==='custom'?'사용자지정':'월구독'}</span>}
                      </div>
                      <div style={{ fontSize:13, fontWeight:500, color:'#e2e8f0' }}>{e.memo||e.cat}</div>
        ...
        {tab==='subscription'&&<>
          <div style={{ background:'rgba(124,58,237,0.06)', border:'1px solid rgba(124,58,237,0.2)', borderRadius:14, padding:'20px 22px', marginBottom:18 }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12, marginBottom:16 }}>
              <div style={{ background:'rgba(255,255,255,0.03)', borderRadius:9, padding:12 }}>
                ...
                <div style={{ fontSize:11, color:'#475569', marginTop:3 }}>{monthlySubs.length}개 · 연 {fmt(monthlyTotal*12)}</div>
              </div>
              <div style={{ background:'rgba(255,255,255,0.03)', borderRadius:9, padding:12 }}>
          ...
          {alertSubs.length>0&&(
            <div style={{ background:'rgba(251,191,36,0.05)', border:'1px solid rgba(251,191,36,0.2)', borderRadius:12, padding:'14px 16px', marginBottom:16 }}>
              ...
              {alertSubs.map(s=>(
                <div key={s.id} style={{ display:'flex', alignItems:'center', gap:10, marginBottom:6, fontSize:13 }}>
                  <span style={{ color:'#e2e8f0', flex:1 }}>{s.name}</span>
                  <span style={{ color:'#64748b', fontSize:12 }}>{fmt(s.amount)}</span>
                  <StatusBadge sub={s}/>
                </div>
              ))}
            </div>
          ...
          {expiredSubs.length>0&&(
            <div style={{ background:'rgba(239,68,68,0.05)', border:'1px solid rgba(239,68,68,0.2)', borderRadius:12, padding:'14px 16px', marginBottom:16 }}>
              ...
              {expiredSubs.map(s=>(
                <div key={s.id} style={{ display:'flex', alignItems:'center', gap:10, marginBottom:4, fontSize:13 }}>
          ...
          <div style={cardS}>
            ...
            <div style={{ padding:'12px 14px', background:'rgba(83,74,183,0.06)', border:'1px solid rgba(83,74,183,0.15)', borderRadius:9, marginBottom:10 }}>
              <label style={{ display:'flex', alignItems:'center', gap:8, cursor:'pointer', marginBottom:fxForm.isSubscription?10:0 }}>
                ...
                <span style={{ fontSize:13, fontWeight:600, color:'#a78bfa' }}>구독 서비스</span>
                <span style={{ fontSize:11, color:'#64748b' }}>구독 관리 탭에도 표시됩니다</span>
              </label>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                  ...
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
                    ...
                    {fxForm.subPeriod!=='custom' ? (
                      ...
                      <div><label style={lbl}>&nbsp;</label><div style={{ fontSize:11, color:'#64748b', paddingTop:10 }}>아래에서 기간 설정</div></div>
                    )}
                  </div>
                  {fxForm.subPeriod==='custom'&&(
                    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
      ...
      {subModal&&<Modal onClose={()=>setSubModal(null)}>
        <SubForm initial={subModal==='add'?null:subModal} onSave={form=>saveSub(form,subModal!=='add'&&subModal.id?subModal.id:null)} onClose={()=>setSubModal(null)}/>
      </Modal>}
