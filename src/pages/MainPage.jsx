...
import { logActivity, formatDate, dueDateStatus, addNotification } from '../utils.js'
...
function ItemForm({ initial, onSave, onClose }) {
  ...
  const [form, setForm] = useState({ bigCat:bigCatId, category:initial?.category||(SUB_CATS[bigCatId]?.[0]||''), page:initial?.page||'', desc:initial?.desc||'', detail:initial?.detail||'', status:initial?.status||'대기', assignee:initial?.assignee||'', urgent:initial?.urgent||false, dueDate:initial?.dueDate||'', id:initial?.id })
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
      ...
      <div><label style={{fontSize:11,fontWeight:700,color:'#64748b',display:'block',marginBottom:5}}>마감일 (선택)</label>
        <input type="date" value={form.dueDate} onChange={e=>set('dueDate',e.target.value)} style={iS}/>
        {form.dueDate && <div style={{marginTop:5,fontSize:11,color: dueDateStatus(form.dueDate)?.color||'#64748b'}}>{dueDateStatus(form.dueDate)?.label}</div>}
      </div>
    ...
    <div style={{borderBottom:'1px solid #192640'}}>
      <div style={{display:'flex',alignItems:'center',gap:12,padding:'13px 16px',transition:'background 0.1s'}}
        ...
        <div style={{flex:1,minWidth:0}}>
          ...
          {item.dueDate && (() => { const ds=dueDateStatus(item.dueDate); return ds ? <span style={{display:'inline-block',marginTop:4,fontSize:11,padding:'1px 8px',borderRadius:50,background:ds.bg,border:`1px solid ${ds.border}`,color:ds.color,fontWeight:600}}>📅 {ds.label}</span> : null })()}
          <div style={{marginTop:4,fontSize:11,color:'#3d4f6b'}}>
            작성자: {ACCOUNTS.find(a=>a.id===item.createdBy)?.name||'알 수 없음'}
            {item.createdAt && <span style={{marginLeft:6}}>{typeof item.createdAt.toDate==='function' ? item.createdAt.toDate().toLocaleDateString('ko-KR') : ''}</span>}
          </div>
...
export default function MainPage({ user, navigate, onLogout, currentPage }) {
  ...
  const handleSave = async (form) => {
    ...
    const prevItem = items.find(i=>i.id===form.id)
    const data = { bigCat:form.bigCat, category:form.category||'', page:form.page||'', desc:form.desc, detail:form.detail||'', status:form.status||'대기', assignee:form.assignee||'', urgent:form.urgent||false, imageUrl:form.imageUrl||'', dueDate:form.dueDate||'' }
    ...
    // 담당자에게 알림
    if (form.assignee && form.assignee !== user.id) {
      const isNewAssign = isNew || (prevItem?.assignee !== form.assignee)
      if (isNewAssign) {
        await addNotification({ db, toUserId:form.assignee, type:'assign',
          title:'담당 업무 배정', body:`"${form.desc.slice(0,40)}" 업무가 배정되었습니다`, itemId:id })
      }
    }
