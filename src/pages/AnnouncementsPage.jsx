...
export default function AnnouncementsPage({ user, navigate, onLogout, currentPage, onRefreshUser }) {
  ...
  const [modal, setModal] = useState(null) // null | 'add' | item객체(수정)
  ...
  const openEdit = (item) => {
    setForm({ title:item.title, content:item.content, pinned:item.pinned||false })
    setModal(item)
  }
  ...
  const handleSave = async () => {
    ...
    if (isEdit) {
      await updateDoc(doc(db, 'announcements', modal.id), {
        title:form.title, content:form.content, pinned:form.pinned,
        updatedAt:serverTimestamp(), updatedByName:user.name
      })
      await logActivity({ userId:user.id, userName:user.name, action:'공지사항 수정', itemDesc:form.title })
      showToast('✅ 공지사항이 수정되었습니다')
    } else {
      await addDoc(collection(db, 'announcements'), {
        ...form, createdBy:user.id, createdByName:user.name, createdAt:serverTimestamp()
      })
  ...
  const togglePin = async (id, current) => {
    ...
    showToast(!current ? '📌 상단 고정되었습니다' : '고정이 해제되었습니다')
  }
      ...
      <div style={{ display:'flex', alignItems:'flex-start', gap:10 }}>
        <div style={{ flex:1 }}>
          ...
          <div style={{ fontSize:11, color:'#3d4f6b', marginBottom:10 }}>
            ...
            {item.updatedAt && <span style={{ marginLeft:8, color:'#2d3f6e' }}>(수정됨 · {item.updatedByName})</span>}
          </div>
          <p style={{ fontSize:13, color:'#94a3b8', lineHeight:1.8, whiteSpace:'pre-wrap' }}>{item.content}</p>
        </div>
        {user.role === 'admin' && (
          <div style={{ display:'flex', gap:6, flexShrink:0 }}>
            <button onClick={()=>togglePin(item.id, item.pinned)} title={item.pinned?'고정 해제':'상단 고정'}
              style={{ padding:'4px 9px', background:item.pinned?'rgba(245,158,11,0.15)':'rgba(255,255,255,0.05)', border:`1px solid ${item.pinned?'rgba(245,158,11,0.3)':'#1e2e50'}`, borderRadius:6, color:item.pinned?'#fcd34d':'#64748b', cursor:'pointer', fontSize:12 }}>
              {item.pinned?'📌':'📍'}
            </button>
            <button onClick={()=>openEdit(item)}
              style={{ padding:'4px 9px', background:'rgba(59,130,246,0.1)', border:'1px solid rgba(59,130,246,0.2)', borderRadius:6, color:'#93c5fd', cursor:'pointer', fontSize:12 }}>
              ✏️
            </button>
            <button onClick={()=>handleDelete(item.id, item.title)}
              style={{ padding:'4px 9px', background:'rgba(239,68,68,0.08)', border:'none', borderRadius:6, color:'#f87171', cursor:'pointer', fontSize:12 }}>
              🗑️
            </button>
          </div>
        )}
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#0d1526', fontFamily:"'Noto Sans KR',sans-serif" }}>
      ...
      <div style={{ maxWidth:800, margin:'0 auto', padding:'28px 20px' }}>
        <PageHeader title="📣 공지 / 앱 정보" onBack={()=>navigate('main')}>
          {mainTab==='announcements' && user.role === 'admin' && (
            <button onClick={openAdd} style={{ padding:'9px 18px', background:'linear-gradient(135deg,#2563eb,#1d4ed8)', border:'none', borderRadius:9, color:'#fff', fontWeight:700, fontSize:13, cursor:'pointer', fontFamily:"'Noto Sans KR',sans-serif" }}>+ 공지 작성</button>
          )}
        </PageHeader>

        {/* 메인 탭 */}
        <div style={{ display:'flex', borderBottom:'1px solid #1e2e50', marginBottom:24 }}>
          {[['announcements','📣 공지사항'],['info','🏢 앱 / 사업자 정보']].map(([t,l])=>(
            <button key={t} onClick={()=>setMainTab(t)} style={{ padding:'9px 20px', border:'none', background:'transparent', fontSize:13, fontWeight:600, cursor:'pointer', color:mainTab===t?'#93c5fd':'#4a5568', borderBottom:`2px solid ${mainTab===t?'#3b82f6':'transparent'}`, marginBottom:-1, fontFamily:"'Noto Sans KR',sans-serif" }}>{l}</button>
          ))}
        </div>
          ...
          <>
            ...
            <div>
              ...
              {normal.length === 0 && pinned.length === 0
                ? <Empty icon="📣" message="등록된 공지사항이 없습니다" />
                : normal.length > 0 && <SectionCard>{normal.map(a => <Card key={a.id} item={a} />)}</SectionCard>
              }
            </div>
          </>
        )}
        ...
        {/* 앱/사업자 정보 탭 */}
        {mainTab==='info' && <InfoPage user={user}/>}
      </div>

      {modal && (
        <Modal onClose={()=>setModal(null)}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20 }}>
            <h3 style={{ fontSize:17, fontWeight:700, color:'#f1f5f9' }}>{isEdit ? '✏️ 공지사항 수정' : '📣 공지사항 작성'}</h3>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
            ...
            <div style={{ display:'flex', gap:10, marginTop:4 }}>
              ...
              <button onClick={handleSave} style={{ flex:2, padding:11, border:'none', borderRadius:9, background:'linear-gradient(135deg,#2563eb,#1d4ed8)', color:'#fff', fontWeight:700, fontSize:14, cursor:'pointer', fontFamily:"'Noto Sans KR',sans-serif" }}>
                {isEdit ? '수정하기' : '등록하기'}
              </button>
            </div>
          </div>
